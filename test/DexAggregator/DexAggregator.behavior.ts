...
export function shouldAddANewDexAdapter(): void {
  ...
  it("should update a Dex adapter", async function () {
    ...
    // Act
    await this.dexAggregator.connect(this.signers.admin).updateDexAdapter(0, this.signers.user1.address, "Dex2", false);

    // Assert
    const adapter = await this.dexAggregator.connect(this.signers.admin).getDexAdapter(0);
    expect(adapter[0]).to.equal(this.signers.user1.address);
    expect(adapter[1]).to.equal("Dex2");
    expect(adapter[2]).to.equal(false);
  });
}
